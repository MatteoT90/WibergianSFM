...
bool GlobalSfMReconstructionEngine_RelativeMotions::Process(
        ...
        if (!Adjust(vec_rows, g, vec_cols, h, vec_grad, i))
        {
...
bool GlobalSfMReconstructionEngine_RelativeMotions::Process(std::string sOutDir) {
...
bool GlobalSfMReconstructionEngine_RelativeMotions::Preprocess() {
        ...
        )
{
  ...
  b_BA_Status = bundle_adjustment_obj.Adjust(sfm_data_, vec_rows, g, vec_cols, h, vec_grad, i);
  if (b_BA_Status && !sLogging_file_.empty())
  {
    Save(sfm_data_,
...
bool GlobalSfMReconstructionEngine_RelativeMotions::Adjust()
    {
        ...
        if (b_BA_Status)
        {
            ...
            {
                Save(sfm_data_,
                     stlplus::create_filespec(stlplus::folder_part(sLogging_file_), "structure_01_refine_RT_Xi", "bin"),
                     ESfM_Data(EXTRINSICS | STRUCTURE));
            }
        }
        ...
        if (b_BA_Status && ReconstructionEngine::intrinsic_refinement_options_ != Intrinsic_Parameter_Type::NONE) {
            ...
            {
                Save(sfm_data_,
                     stlplus::create_filespec(stlplus::folder_part(sLogging_file_), "structure_02_refine_KRT_Xi", "bin"),
                     ESfM_Data(EXTRINSICS | STRUCTURE));
            }
        }
...
bool GlobalSfMReconstructionEngine_RelativeMotions::Adjust(std::string sOutDir)
{
    // Refine sfm_scene (in a 3 iteration process (free the parameters regarding their incertainty order)):
    ...
    Save(sfm_data_,
         stlplus::create_filespec(sOutDir, "sfm_data_before_final_BA", ".bin"),
         ESfM_Data(ALL));
    return b_BA_Status;
}
